...
qiankun中有两种沙箱实现, `快照沙箱`和`代理沙箱`. 当浏览器支持 `proxy` 时优先使用代理沙箱, 否则自动降级到快照沙箱.
...
全局变量可以以上方式直接复原，但是`事件监听`和`定时器`需要特殊处理：用 addEventListener 添加的事件，需要用 removeEventListener 方法来移除，定时器也需要特殊函数才能清除。所以它重写了事件绑定/解绑和定时器相关函数。
