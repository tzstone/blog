...
- 合并 CSS, JavaScript
- 合并小图片, 使用雪碧图(`webpack-spritesmith`)
...
- preload: 声明式的 fetch, 允许你强制浏览器去请求资源, 同时不会堵塞 docment 的`onload`事件(一般是当前页面需要的资源)
  ...
  - 让`preload`样式立即生效([其他](http://yoavweiss.github.io/link_htmlspecial_16/#53))
...
- prefetch: 提示浏览器某个资源将来可能需要, 但把是否下载以及什么时候下载的决定权交给浏览器(一般是其他页面需要的资源)
  - `<link rel=“prefetch”>`
  - 缓存: 同`preload`
...
- dns-prefetch: 在浏览器请求资源之前启动托管资源的每个域的 DNS 解析的过程, 减少请求资源时 DNS 的解析时间
  ...
  - chrome 默认情况下不会 prefetch HTTPS 页面中超链接的域名, 可以通过 `x-dns-prefetch-control` 取消限制(`content="off"`则会关闭 HTTP 页面超链接默认的 prefetch)
