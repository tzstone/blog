...
- 压缩图片
  ...
  - `<picture>`元素: 允许在其内部设置多个`<source>`标签, 以指定不同的图像文件名, 根据不同的条件进行加载, 这些条件可以是视窗当前的高度(viewport), 宽度(width), 方向(orientation), 像素密度(dpr)等(可以加载 webp 格式图片)
    - 最后的 `<img>` 元素是在`<picture>`内部用来当浏览器不支持时或者没有源标签匹配时的显示
    - ```html
      <picture>
